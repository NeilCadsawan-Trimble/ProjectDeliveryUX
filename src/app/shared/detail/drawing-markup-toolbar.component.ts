...
@Component({
  ...
  template: `
    <div class="absolute bottom-3 left-0 right-0 z-10 flex justify-center pointer-events-none">
      <div class="flex items-center gap-1 toolbar-dark-always rounded-lg px-3 py-1.5 shadow-toolbar pointer-events-auto">
        @for (tool of tools(); track tool.label) {
          <div class="tool-btn w-8 h-8 rounded flex items-center justify-center cursor-pointer transition-colors duration-150 relative"
            [class.tool-btn-active]="activeTool() === tool.label"
            role="button" tabindex="0" [attr.aria-label]="tool.label"
            (click)="selectTool(tool.label)" (mousedown)="$event.stopPropagation()">
            @if (tool.svgSrc === 'draw') {
              <svg class="w-5 h-5" viewBox="12 12 24 24" fill="none" aria-hidden="true">
              ...
              <i class="modus-icons text-2xl" aria-hidden="true">{{ tool.icon }}</i>
            }
            <svg class="absolute bottom-0.5 right-0.5 w-1.5 h-1.5 tool-corner" viewBox="0 0 6 6" fill="currentColor" aria-hidden="true">
