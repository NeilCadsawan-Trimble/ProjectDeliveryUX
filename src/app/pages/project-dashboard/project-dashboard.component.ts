    ...
    <ng-template #drawingToolbar>
      <div class="absolute bottom-3 left-0 right-0 z-10 flex justify-center pointer-events-none">
        <div class="flex items-center gap-1 bg-card border-default rounded-lg px-3 py-1.5 shadow-toolbar pointer-events-auto">
          @for (tool of drawingTools; track tool.icon) {
            @if (tool.separator) {
              ...
              <div class="w-8 h-8 rounded flex items-center justify-center cursor-pointer hover:bg-muted transition-colors duration-150 relative"
                [class.bg-muted]="activeDrawingTool() === tool.label"
                role="button" tabindex="0" [attr.aria-label]="tool.label"
                (click)="activeDrawingTool.set(tool.label)">
                @if (tool.svgSrc === 'draw') {
          ...
          <div class="mt-auto border-top-default">
            <div class="relative">
              <div
                ...
                [title]="detailDrawing() ? 'Reset Layout' : 'Reset options'"
                role="button"
                [attr.aria-label]="detailDrawing() ? 'Reset Layout' : 'Reset options'"
...
export class ProjectDashboardComponent implements OnInit, AfterViewInit {
  ...
  readonly drawingTools: { icon: string; label: string; separator?: boolean; svgSrc?: string }[] = [
