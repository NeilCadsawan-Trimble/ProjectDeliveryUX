...
**Version**: 1.1
**Date**: April 7, 2026
**Status**: Active Development (96% complete -- 175/183 items)
...
| ------------------------------------ | ------------------------------------------------------------ | --------------------------------------------------------------------- |
| **Frank Mendoza** (Dashboard User)   | Portfolio Manager -- oversees multiple construction projects | Home dashboard, cross-project financials, portfolio-level AI analysis |
| **Bert Humphries** (Project Manager) | Manages all 8 projects end-to-end                            | Project detail dashboard, records, milestones, tasks, budget          |
...
| HOME-12 | KPI cards must be a draggable/resizable widget (not a locked header) with compact vertical display                            | P1       |
| HOME-13 | Widgets must support area-adaptive content that dynamically fills available pixel height using prioritized content blocks     | P1       |
| HOME-14 | Widgets must support progressive text scaling across 3 tiers (default, medium, large) based on column span and height         | P2       |
...
| PROJ-07 | Provide a "Sort by Priority" action in the layout menu that reorders tiles by urgency (critical/warning count) | P1       |
| PROJ-08 | Risk badges must use `coreWarningCount` (excluding routine change orders) for accurate severity determination  | P1       |
...
| PD-08 | RFI and Submittal widgets must display compact status summary tiles (Open, Overdue, Upcoming, Closed) when column span <= 5 or on mobile | P1       |
...
| ID      | Requirement                                                                                                                                        | Priority |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| AUTH-01 | Gate all application routes behind Trimble ID authentication using OAuth 2.0 PKCE flow                                                             | P0       |
| AUTH-02 | Provide a dedicated login page with Trimble branding that initiates the OAuth flow                                                                 | P0       |
| AUTH-03 | Provide an auth callback page that handles token exchange and redirects to the original target route                                               | P0       |
| AUTH-04 | Auth callback must use `NgZone.run()` to re-enter Angular's zone after token exchange (trimble-id uses axios/CommonJS which exits Zone.js context) | P0       |
| AUTH-05 | Provide a hard-redirect fallback (`window.location.replace`) if Angular soft navigation fails after auth                                           | P0       |
| AUTH-06 | OAuth redirect URIs must be dynamic via `window.location.origin` to work across all deployment environments                                        | P0       |
...
| VM-D09 | Widgets must support area-adaptive content disclosure with pixel-height budgeting and progressive text scaling                     | P1       |
...
| VM-C11 | Canvas content must use fixed positioning with viewport-relative centering (`left: 50%; transform: translateX(-50%)`) to align with the fixed navbar | P0       |
...
| NAV-08 | Sidenav must overlay content via `position: fixed; z-index: 999` -- no content shifting (`pl-60` push) | P0       |
| NAV-09 | Sidenav row heights must be consistent (56px) between collapsed and expanded states                    | P1       |
...
| ID      | Requirement                                                                                                                                          | Priority |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| DATA-10 | Provide live weather data via OpenWeatherMap API through a Vercel Edge Function proxy with 15-minute TTL caching                                     | P1       |
| DATA-11 | Flag weather conditions for potential construction work impact with severity badges and color-coded forecast pills                                   | P1       |
| DATA-12 | Aggregate weather data for portfolio-level view on Home dashboard                                                                                    | P1       |
| DATA-13 | `WeatherService` must implement TTL-based caching, concurrent fetch prevention, and per-city deduplication across projects sharing the same location | P1       |
...
| ID      | Requirement                                                                                                                 | Priority |
| ------- | --------------------------------------------------------------------------------------------------------------------------- | -------- |
| DATA-14 | Dashboard user persona: Frank Mendoza (displayed in navbar greeting and home page welcome)                                  | P1       |
| DATA-15 | Project ownership: Bert Humphries as Project Manager and owner on all 8 projects                                            | P1       |
| DATA-16 | Team rosters must reflect realistic construction role distribution (PM, Superintendent, Senior Engineer, Coordinator, etc.) | P2       |
...
| NFR-15 | All code must pass `npm run test:static` (413+ static compliance tests) before push | P0       |
...
### 9.5 Deployment Infrastructure


| ID     | Requirement                                                                                                                             | Priority |
| ------ | --------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| NFR-18 | Deploy via Vercel with auto-deploy on push to main                                                                                      | P0       |
| NFR-19 | External API calls must go through Vercel Edge Function proxies with TTL-based edge caching                                             | P1       |
| NFR-20 | Production builds must clear Angular build cache (`rm -rf .angular/cache`) before `ng build` to prevent stale route definitions         | P0       |
| NFR-21 | HTML responses must include `Cache-Control: no-cache, no-store, must-revalidate` headers to prevent CDN from serving stale `index.html` | P0       |
| NFR-22 | SPA routing must be handled via Vercel rewrites (all non-API, non-asset paths rewrite to `/index.html`)                                 | P0       |
...
| ~~User authentication~~  | ~~No auth layer~~ -- **Implemented Apr 7**: Trimble ID OAuth 2.0 PKCE gate on all persona routes |
...
| Google Cloud integration | Gmail and Calendar widget integration planned (decided Mar 31) but not yet started               |
...
| Desktop, Canvas Card, and Mobile modes maintain feature parity             | Static parity tests (413+ tests passing)                         |
...
| Unauthenticated users are redirected to Trimble ID login                   | Auth guard blocks all persona routes                             |
| Live weather data loads for all 8 project locations                        | WeatherService fetches from OpenWeatherMap API via Edge Function |
| Vercel production deployment serves fresh content                          | No stale CDN or build cache artifacts                            |
...
| ------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.0     | March 31, 2026 | Initial PRD capturing 125/133 requirements across all feature areas                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 1.1     | April 7, 2026  | Added: Section 3.7 Authentication (AUTH-01 through AUTH-06), area-adaptive widgets (HOME-12/13/14), Sort by Priority (PROJ-07/08), compact widget mode (PD-08), canvas fixed positioning (VM-C11), adaptive content in desktop (VM-D09), sidenav overlay (NAV-08/09), live weather API (DATA-10 updated, DATA-13), persona data (DATA-14/15/16), deployment infrastructure (NFR-18 through NFR-22), updated static test count (325 to 413+). Moved "User authentication" from Out of Scope to implemented. Added 3 new success criteria. Updated persona names in target users. |
